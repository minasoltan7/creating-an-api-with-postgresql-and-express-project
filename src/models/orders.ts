...
export class OrderModel {
  ...
  async addProduct(quantiy: number, orderId: number, productId: number): Promise<Order[]> {
    try {
      const conn = await Client.connect()
      const sql =
        'INSERT INTO order_products (quantity,order_id,product_id) VALUES($1,$2,$3) RETURNING *'
      const result = await conn.query(sql, [quantiy, orderId, productId])
      conn.release()
      return result.rows[0]
    } catch (err) {
  ...
  async userCurrentOrders(id: number): Promise<Order[]> {
    try {
      const conn = await Client.connect()
      const sql =
        "SELECT order_id,product_id,quantity,user_id,status  FROM orders JOIN order_products ON orders.id=order_products.order_id  WHERE user_id=($1)  AND status='active' "
      const result = await conn.query(sql, [id])
      conn.release()
      return result.rows[0]
    } catch (err) {
      throw new Error(`Could not get current order of id ${id}: ${err}`)
    }
  }
}
